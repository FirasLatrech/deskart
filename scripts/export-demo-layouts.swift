...
import Foundation
// Export full placements (shape + parked) in *screen* coords for the animation.
...
let specs: [(String, Shape, String)] = [
    ...
    ("spiral", .spiral, ""), ("smiley", .smiley, ""), ("infinity", .infinity, ""),
]
for (name, sh, txt) in specs {
    let plan = Placer.plan(shape: sh, text: txt, names: names, mapper: m)
    // Keep icon identity: names[i] -> position, in draw order
    var pts: [[Double]] = []
    var kinds: [Int] = []
    let shapeSet = Set(plan.previewShape.map { "\(Int($0.x)),\(Int($0.y))" })
    for n in names {
        guard let p = plan.placements[n] else { continue }
        pts.append([Double((p.x - r.minX) / r.width), Double((p.y - r.minY) / r.height)])
        kinds.append(shapeSet.contains("\(Int(p.x)),\(Int(p.y))") ? 1 : 0)
    }
    out[name] = ["points": pts, "kinds": kinds, "shapeCount": plan.shapeCount, "parked": plan.parkedCount]
}
out["_meta"] = ["w": Double(r.width), "h": Double(r.height)]
